...
public class HttpServerApp {
    ...
    public HttpServerApp(UserService users, Dispatcher dispatcher, FileCatalog catalog, List<FileContainer> containers) {
        this.users = users; this.dispatcher = dispatcher; this.catalog = catalog; this.containers = containers;
    }
    ...
    private void download(HttpExchange ex) throws IOException {
        ...
        User u = users.auth(header(ex, "X-User"), header(ex, "X-Pass"));
        if (u == null) { respondText(ex, 401, "Unauthorized"); return; }
        String filename = query(ex, "filename"); int priority = parseInt(query(ex, "priority"), 5);
        if (filename == null) { respondText(ex, 400, "filename required"); return; }
    ...
    private void share(HttpExchange ex) throws IOException {
        ...
        User u = users.auth(header(ex, "X-User"), header(ex, "X-Pass"));
        if (u == null) { respondText(ex, 401, "Unauthorized"); return; }
        String filename = query(ex, "filename"); String to = query(ex, "to"); String perm = query(ex, "perm");
    ...
    private void userUpdate(HttpExchange ex) throws IOException {
        if (!"POST".equalsIgnoreCase(ex.getRequestMethod())) { respondText(ex, 405, "Use POST"); return; }
        User u = users.auth(header(ex, "X-User"), header(ex, "X-Pass")); if (u == null) { respondText(ex, 401, "Unauthorized"); return; }
    ...
    private void adminUserDelete(HttpExchange ex) throws IOException {
        User u = users.auth(header(ex, "X-User"), header(ex, "X-Pass")); if (u == null || u.role != User.Role.ADMIN) { respondText(ex, 401, "Admin required"); return; }
    ...
    private void addContainer(HttpExchange ex) throws IOException {
        User u = users.auth(header(ex, "X-User"), header(ex, "X-Pass")); if (u == null || u.role != User.Role.ADMIN) { respondText(ex, 401, "Admin required"); return; }
        String id = query(ex, "id"); if (id == null) { respondText(ex, 400, "id required"); return; }
...
private void removeContainer(HttpExchange ex) throws IOException {
    ...
    synchronized (catalog) {
        for (String f : catalog.filesOn(c)) {  // snapshot set; safe to iterate
    ...
    private static String header(HttpExchange ex, String k) {
        List<String> vals = ex.getRequestHeaders().get(k); return (vals == null || vals.isEmpty()) ? null : vals.get(0);
    }
    private static String query(HttpExchange ex, String key) {
        String q = ex.getRequestURI().getRawQuery(); if (q == null) return null;
        for (String part : q.split("&")) {
            int i = part.indexOf('='); if (i > 0) {
    ...
    private static byte[] readAll(InputStream is) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream(); byte[] buf = new byte[8192]; int r;
    ...
    private static String toJson(Object v) {
        ...
        if (v instanceof Collection) {
            ...
            for (Object o : (Collection<?>) v) { if (!first) sb.append(','); first = false; sb.append(toJson(o)); }
